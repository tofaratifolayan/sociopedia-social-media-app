...
export default function DisplayProducts(props) {
  ...
  function onAddToCartClick(product) {
    let order = new Order(null, product.title, product.price, product.amount, true, null);
    props.onAddToCartClick(order);
  }
  
  // let [num, setNum] = useState(0);
  let initialstate = products.map(product => ({id: product.id,
                                                title: product.title,
  ...
  const [initialObj, setInitialObj] = useState(initialstate)

  function incNum(product) {
    if (product.amount < 10) {
      const amount = product.amount + 1;
      let newState = initialObj.filter(prod => prod.id === product.id)
      let newProd = {id: product.id,
                    title: product.title,
                    description: product.description,
                    price: product.price,
                    downloadUrls: product.downloadUrls, 
                    amount: amount}
      setInitialObj([...newState, newProd])
    }
  };


  function decNum(product) {
    if (product.amount > 0) {
      const amount = product.amount - 1;
      let newState = initialObj.filter(prod => prod.id !== product.id)
      let newProd = {id: product.id,
            ...
            {initialObj.map((product) => (
              <div key={product.id} className="container-fluid">
                <div className="row">
                  ...
                  >
                    <Imagegallery
                      productUrls={product.downloadUrls}
                      ...
                      <div className="modal" tabIndex="-1" id="myModal">
                        <div className="modal-dialog">
                          <div className="modal-body alert alert-success">
                            <p>
                              {product.amount} {product.title} added successfully!!
                            </p>
                          </div>
                        </div>
                      </div>
